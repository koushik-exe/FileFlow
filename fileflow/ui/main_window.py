...
from fileflow.constants import C, FONT_FAMILY, file_types, MONTHS
from fileflow.core import RunContext, phase1_organize, phase2_verify, revert_organize, hash_map
from fileflow.utils import get_folder_size, format_size
from fileflow.ui.widgets import (
    REDUCE_MOTION,
    ease_out_cubic,
    Animator,
    make_button_premium,
...
class FileFlowApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        self._config_path = os.path.join(os.getcwd(), "fileflow_config.json")
    ...
    def _start_god_mode(self):
        ...
        # Create RunContext from user input
        log_file = os.path.join(dst, "god_mode_log.txt")
        report_file = os.path.join(dst, "deep_verification.txt")
        self._ctx = RunContext(src=src, dst=dst, log=log_file, report=report_file)
    ...
    def _worker(self):
        ...
        before_messy = get_folder_size(ctx.src)
        ...
        after_messy  = get_folder_size(ctx.dst)

        # Store in shared stats dict
        self._stats.update({
    ...
    def _build_overview_page(self):
        ...
        self.card_src = HeroFolderCard(hero_frame, "Source Folder", "📂", "", C.ACCENT_BLUE)
        ...
        self.card_dst = HeroFolderCard(hero_frame, "Destination Folder", "🗂️", "", C.ACCENT_PURPLE)
